...
trP <- Sys.time()
res_reverse_G_kPsi <- foreach(i=1:nrep, .packages = c("RRembo", "DiceKriging"), .combine = 'rbind') %dopar% {
  set.seed(i)
  res <- easyREMBO(par = runif(d), fn = ftest, budget = budget, lower = lower, upper = upper,
                   ii = mat_effective[i,],
...
trY <- Sys.time()
res_reverse_G_kY <- foreach(i=1:nrep, .packages = c("RRembo", "DiceKriging"), .combine = 'rbind') %dopar% {
  set.seed(i)
  res <- easyREMBO(par = runif(d), fn = ftest, budget = budget, lower = lower, upper = upper,
                   ii = mat_effective[i,],
...
trX <- Sys.time()
res_reverse_G_kX <- foreach(i=1:nrep, .packages = c("RRembo", "DiceKriging"), .combine = 'rbind') %dopar% {
  set.seed(i)
  res <- easyREMBO(par = runif(d), fn = ftest, budget = budget, lower = lower, upper = upper,
                   ii = mat_effective[i,],
...
# Plot final results
boxplot(res_RO[,budget] - fstar, 
        res_standard_kY[,budget] - fstar, res_standard_kX[,budget] - fstar, res_standard_kPsi[,budget] - fstar,
        res_reverse_G_kY[,budget] - fstar, res_reverse_G_kX[,budget] - fstar, res_reverse_G_kPsi[,budget] - fstar)

# Post-treatment for progression
rRO <- res_RO
rskY <- res_standard_kY; rskX <- res_standard_kX; rskP <- res_standard_kPsi
rrkY <- res_reverse_G_kY; rrkX <- res_reverse_G_kX; rrkP <- res_reverse_G_kPsi
